describe('Axe testing', () => {
    ...
    before(() => {
        cy.intercept('POST', 'https://sentry.gc.nav.no/**', {}).as(
            'sentry',
        );
        cy.intercept('POST', 'https://amplitude.nav.no/**', {}).as(
            'amplitude',
        );
        cy.intercept(
            'GET',
            'https://www.nav.no/dekoratoren/client**',
            {},
        ).as('dekoratoren');
        ...
        cy.intercept('GET', formPath, { fixture: `form.json` }).as(
            'getForm',
        );
        cy.intercept('GET', `${formPath}/vedlegg/**/fil`, []).as(
            'fil',
        );
