...
const LagringsProsessContext = createContext<LagringsProsessContextType | null>(null);
...
export const useLagringsProsessContext = () => {
  ...
  if (!lagringsProsessContext) {
    throw new Error('Mangler LagringsProsessProvider, når useLagringsProsessContext kalles');
  }
  return lagringsProsessContext;
};

export const LagringsProsessProvider = ({ children }: LagringsProsessProviderProps) => {
  const [aktiveLagringsProsesser, setAktiveLagringsProsesser] = useState<Promise<unknown>[]>([]);
  const aktiveLagringsProsesserRef = useRef<Promise<unknown>[]>([]);

  const leggTilLagringsProsess = useCallback(<T,>(promise: Promise<T>) => {
